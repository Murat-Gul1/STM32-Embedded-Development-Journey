...
// Screenbuffer
static uint8_t ScreenBuffer_A[SSD1306_BUFFER_SIZE];
static uint8_t ScreenBuffer_B[SSD1306_BUFFER_SIZE];
static uint8_t *SSD1306_Buffer = ScreenBuffer_A;
static uint8_t *DMA_Buffer = ScreenBuffer_B;
// Screen object
...
void ssd1306_UpdateScreen_DMA(void) {
    // 1. Önceki transferin bitmesini bekle (Kilitlenmeyi önler)
    while (HAL_I2C_GetState(&SSD1306_I2C_PORT) != HAL_I2C_STATE_READY);

    // 2. Tampon Takası (Double Buffering)
    uint8_t *temp = SSD1306_Buffer;
    SSD1306_Buffer = DMA_Buffer;
    DMA_Buffer = temp;

    // --- KRİTİK: EKRANIN VERİYİ NEREYE YAZACAĞINI SIFIRLA ---
    // Bu komutlar ekranın içindeki yazım penceresini 0,0'dan 127,63'e ayarlar.
    ssd1306_WriteCommand(0x21); // Sütun (X) ayarı
    ssd1306_WriteCommand(0);    // Başlangıç: 0
    ssd1306_WriteCommand(127);  // Bitiş: 127

    ssd1306_WriteCommand(0x22); // Sayfa (Y) ayarı
    ssd1306_WriteCommand(0);    // Başlangıç: 0
    ssd1306_WriteCommand(7);    // Bitiş: 7 (8 sayfa * 8 piksel = 64)
    // -----------------------------------------------------

    // 3. Veriyi gönder
    HAL_I2C_Mem_Write_DMA(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, 1, DMA_Buffer, SSD1306_BUFFER_SIZE);
}
